...
export default function Register() {
  ...
  let validationSchema=Yup.object({
    name:Yup.string().min(3,'name shuld be grater than 3 char').max(20,'name shuld be less than 20 char').required('name is required'),
    email:Yup.string().required('email is required and  writen  as moh.eha@gmail.com').matches(/^\w{3,}.?\w{4,}?@(\w{3,}).(\w{3,})$/i),
    password:Yup.string().required('password is required and sould have from 6-10 number and capital &small liter').matches(/^\d{6,10}[A-Z]{1}[a-z]{1}$/i),
    rePassword:Yup.string().required('rePassword is required').matches(/^\d{6,10}[A-Z]{1}[a-z]{1}$/i).oneOf([Yup.ref('password')]),
     ...
     <div className='my-5'>
      <div className='m-auto w-75'>
        ...
        <form className='my-3' onSubmit={formik.handleSubmit}>
          ...
          <input onBlur={formik.handleBlur} value={formik.values.name} onChange={formik.handleChange} className='mb-2 form-control' type='name' name='name' id='name' />
          ...
          <label className='mb-2' htmlFor='password'>Pass word:</label>
          <input onBlur={formik.handleBlur} value={formik.values.password} onChange={formik.handleChange} className='mb-2 form-control' type='password' name='password' id='password' />

          {formik.errors.password && formik.touched.password ? <div className='alert alert-danger'>
             {formik.errors.password}
           </div>:null}

          <label className='mb-2' htmlFor='rePassword'>Repass word :</label>
          <input onBlur={formik.handleBlur} value={formik.values.rePassword} onChange={formik.handleChange} className='mb-2 form-control' type='password' name='rePassword' id='rePassword' />
          ...
          {formik.errors.phone && formik.touched.phone ? <div className='alert alert-danger'>
           ...
           {loading?
           <button disabled type='btn' className='ms-auto d-block btn bg-main text-white'><i className="fa-solid fa-spinner fa-spin"></i></button>:
           <button disabled={loading} type='botton' className='ms-auto d-block btn bg-main text-white'>Register</button>}
